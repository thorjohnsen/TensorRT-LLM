...
class NGramPoolManager(BaseResourceManager):
    ...
    def prepare_resources(self, scheduled_batch: ScheduledRequests):
        if self.is_overlap_scheduler:
            return
        self._prepare_resources_impl(scheduled_batch, None)

    def prepare_draft_tokens(self, scheduled_batch: ScheduledRequests,
                             new_tokens: torch.Tensor):
        self._prepare_resources_impl(scheduled_batch, new_tokens)

    def _prepare_resources_impl(self, scheduled_batch: ScheduledRequests,
                                new_tokens: torch.Tensor):
    ...
    def _get_draft_tokens(
        ...
        if prefix_len >= max_sequence_length - 1:  # Skip search if prefix is long enough
            return None
