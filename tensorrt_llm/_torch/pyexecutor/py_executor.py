    ...
    def _executor_loop_overlap(self):
        ...
        with self._profiler() as profile_step:
            ...
            while not got_finish_signal or len(self.active_requests) > 0:
                ...
                spec_config = self.model_engine.spec_config
                is_ngram = spec_config is not None and spec_config.spec_dec_mode.is_ngram(
                )
                if is_ngram:
                    self._prepare_draft_requests()
                ...
                if scheduled_batch.batch_size > 0:
                    ...
                    if is_ngram:
                        previous_tensors_host = self.previous_batch and self.previous_batch.decoder_state.new_tensors_host
                        self.resource_manager.resource_managers[
                            'spec_resource_manager'].prepare_draft_tokens(
                                scheduled_batch, previous_tensors_host)
